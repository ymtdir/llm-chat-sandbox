...
- `/DESIGN.md` ではAppleのSF Proフォントを参照していますが、実装時にはライセンス制約を考慮してシステムフォントスタック（`-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif`）を使用すること
- これによりApple端末ではSF Proが、その他の端末では各プラットフォームのネイティブフォントが自動的に適用されます
